...
void main() 
{
	vec3 L = normalize(lightDirection - vPosition.xyz);	
	...
	vec3 hemisphere = 0.5f * mix(color3,color2,hemDot) + 0.5f;
	
	//Ambient Light
	vec3 Ambient = (Ka) * (Ia * 0.01f) *  hemisphere;
	//vec3 Ambient = Ia * Ka;
	//Lambert Term
	float LambertTerm =  max(0.0f, dot(N,L)) ;
	//Deffuse
	vec3 Diffuse = Kd * Id * LambertTerm   ;
	
	vec3 Specualar = Is * Ks * specularTerm; // Specular light (1)
	
	FragColor = vec4( Ambient  + Specualar, 1);
}